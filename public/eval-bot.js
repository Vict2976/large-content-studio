...
(function () {
  ...
  try {
    ...
    RESET = location.search.indexOf('evalbot=reset') !== -1;
    if (TEST) {
...
var state = load();
  ...
  try {
    if (!TEST && sessionStorage.getItem(SESSION_FLAG)) {
      quiet('session');
      return;
    }
  } catch (e) {}
  
  if (!TEST && state.lastAsked && Date.now() - state.lastAsked < GLOBAL_COOLDOWN_MS) {
    quiet('cooldown');
    return;
  }
  var questions = [];
