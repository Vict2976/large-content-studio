...
var state = load();
  ...
  function shell(label) {
    ...
    var card = el(
      'div',
      'c',
      'box-sizing:border-box;width:300px;padding:14px 16px;border-radius:12px;background:#1b1b1f;' +
  ...
  function ask(q) {
    ...
    if (TEST) {
      el(
        'span',
        't',
        'display:block;margin:0 0 8px;font-size:11px;font-weight:700;letter-spacing:.04em;' +
          'text-transform:uppercase;color:#ffc46b',
        card,
        'Test mode · not counted',
      );
    }
    
    el('p', 'l', 'margin:0 0 4px;font-size:12px;color:rgba(242,242,245,.6)', card);
    el('p', 'q', 'margin:0 0 12px', card);
    var row = el('div', 'r', 'display:flex;gap:6px', card);
    el('div', 'e', 'display:flex;margin-top:6px;font-size:11px;color:rgba(242,242,245,.55)', card);
    el(
      'button',
      'n',
      BTN + 'margin-top:10px;background:none;border:0;padding:0;font-size:12px;' +
        'color:rgba(242,242,245,.55);text-align:left',
      card,
    ).type = 'button';
    el(
      'button',
      'x',
      BTN + 'margin-top:10px;background:none;border:0;padding:0;font-size:12px;color:rgba(242,242,245,.55)',
      card,
      'No thanks',
    ).type = 'button';
    var lead = wrap.querySelector('.l');
    lead.textContent = q.lead || '';
    lead.style.display = q.lead ? 'block' : 'none';
    wrap.querySelector('.q').textContent = q.ask;
    var row = wrap.querySelector('.r');
    ...
    opts.forEach(function (o) {
      var b = el(
        'button',
        '',
        BTN + 'flex:1;padding:8px 0;border-radius:8px;border:1px solid rgba(255,255,255,.18);' +
      ...
      b.addEventListener('click', function () {
        ...
        record(q, o[1], false);
        if (isLow(q, o[1])) followUp(q, wrap, host);
        else host.remove();
      });
      row.appendChild(b);
    });
    
    var ends = wrap.querySelector('.e');
    if (q.labels && q.labels.length === 2 && q.kind !== 'yes-no') {
      ends.textContent = '';
      el('span', '', 'flex:1', ends, q.labels[0]);
      el('span', '', 'flex:1;text-align:right', ends, q.labels[1]);
      ends.style.display = 'flex';
    } else {
      ends.style.display = 'none';
    }
    
    var na = wrap.querySelector('.n');
    if (q.notApplicable) {
      na.textContent = q.notApplicable;
      na.style.display = 'block';
      na.onclick = function () {
        record(q, undefined, true);
        host.remove();
      };
    } else {
  ...
  function followUp(q, wrap, host) {
    ...
    card.querySelector('.x').textContent = 'Skip';
    box.focus();
  }
